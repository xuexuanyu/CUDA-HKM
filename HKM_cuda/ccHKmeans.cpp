...
//    cout << "cleared kdf: " << this->kdf << endl;
  ...
  else
  {
		...
		//cuknn(dists, ids, 1, data, this->means);
		
  }
  /*for (int i = 0; i < 10; i++)
  {
	  for (int j = 0; j < 64; j++)
		  printf("%d ", means.data.fixed.data.full[i * 64 + j]);
	  printf("\n");
	  printf("\n");
  }*/
...
//  cout << "kmeans:npoints " << data.size() << endl;
  ...
  for (int iter=0; iter<opt.niters; ++iter)
  {
    //compute the current means
	  ...
	  {
		  uint *meansbycuda = new uint[10 * 64];
		  getmeanbycuda(data, ids, meansbycuda);
		  this->means.data.fixed.data.full = (T*)meansbycuda;
	  }*/
